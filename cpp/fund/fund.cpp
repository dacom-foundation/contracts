...
//type: accumulation, expend
  ...
  [[eosio::action]] void fund::addinitial(eosio::name coopname, eosio::name usernane, eosio::asset quantity) { 
    ...
    fundwallet.modify(wal, payer, [&](auto &row){
      row.initial 
    });
  }
  ...
  [[eosio::action]] void fund::spreadamount( eosio::name coopname, eosio::asset quantity) {
    ...
    eosio::check(wal != fundwallet.end(), "Кошелёк невозвратных взносов не найден");
    ...
    fundwallet.modify(wal, payer, [&](auto &w) {
      w.available += remain_amount;  //накопительный счет списания
      w.membership += remain_amount; //индикаторный счет поступивших членских взносов
    });
